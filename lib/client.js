...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CSS_TEXT = [
			...
			".cbc-badge.cbc-badge-err{color:var(--dsw-alias-state-error-primary,#d33);border-color:var(--dsw-alias-state-error-primary,#d33)}",
			// ---- 标签页 ----
		...
		function CbcInput(props) {
			...
			// 密钥输入：密文显示 + 关闭浏览器自动填充/保存（type=password
			// 的框浏览器会提议存入密码管理器，autoComplete=off 抑制）。
			if (props.autoComplete) shared.autoComplete = props.autoComplete;
		...
		// 破坏性操作二次确认（删 Key / 删服务商 / 退出登录共用一套）：第一次
		// 点击只把按钮切成 confirmLabel（如"确认删除"），CONFIRM_WINDOW_MS 内
		// 不再点击自动复位；第二次点击才真正执行 onConfirm。定时器挂在
		// useRef（踩坑 #27：组件局部变量不跨渲染），卸载时清掉。
		// 模块级组件（踩坑：分区内定义的组件每渲染换身份会丢状态/焦点）。
		...
		function ConfirmButton(props) {
			...
			// 行身份变了（如列表重排后同一位置换了对象）就复位，避免"确认"落到别的行。
			useEffect(function () { clearTimer(); setArmed(false); }, [props.confirmKey]);
			var onClick = function () {
				if (!armed) {
					setArmed(true);
					clearTimer();
					confirmTimerRef.current = setTimeout(function () { confirmTimerRef.current = null; setArmed(false); }, CONFIRM_WINDOW_MS);
					return;
				}
				clearTimer();
				setArmed(false);
			...
			return createElement(CbcButton, {
				variant: armed ? "primary" : (props.variant || "ghost"),
				danger: true,
				disabled: props.disabled,
				title: armed ? "再点一次执行；4 秒内不点自动取消" : (props.title || undefined),
				onClick: onClick,
			}, armed ? (props.confirmLabel || "确认删除") : props.children);
		}
		...
		function buildChips(data) {
			...
			if (value.authMode === "oauth") {
				login = oauth.signedIn
					? (oauth.needsReauth ? { tone: "err", text: "OAuth 需重新登录" } : { tone: "ok", text: "OAuth 已登录" })
		...
		function tabBadge(data) {
			...
			var login = value.authMode === "oauth"
				? (oauth.signedIn ? (oauth.needsReauth ? { text: "需重登", tone: "warn" } : { text: "已登录", tone: "ok" }) : { text: "未登录", tone: "warn" })
				: null;
		...
		function LoginSection(props) {
			...
			if (value.authMode === "oauth") {
				...
				if (oauth.signedIn) {
					...
					// 令牌在但已死（刷新被拒 / 刷新令牌过期）：不能再显示"已登录"，
					// 否则用户看到绿灯而聊天全 503（oauth.needsReauth 来自宿主）。
					var dead = oauth.needsReauth === true;
					parts.push(createElement("div", { key: "acct", className: "cbc-listrow" + (dead ? " cbc-reauth" : "") },
						createElement(Dot, { tone: dead ? "err" : "ok" }),
						createElement("span", { className: "cbc-cell-name" }, acct.nickname || acct.uid || "未知账号"),
						createElement("span", { className: "cbc-badge" + (dead ? " cbc-badge-err" : "") }, dead ? "需重新登录" : "已登录"),
					...
					if (dead) {
						parts.push(createElement("p", { key: "reauth", className: "cbc-error" },
							"登录态已失效：" + (oauth.reauthReason || "令牌无法刷新") + "。在此之前经此账号的模型对话 / 搜索 / 生图会全部失败（503）。"));
					}
					...
					parts.push(createElement("div", { key: "out", className: "cbc-addrow" },
						dead ? createElement(CbcButton, { variant: "primary", disabled: oauthBusy, onClick: function () {
							// 与下方"登录 CodeBuddy 账号"同一套：先同步 window.open，
							// 再登出旧态并发起新流程（旧 poll 由宿主 logout() 终止）。
							var win = window.open("", "_blank");
							setOauthBusy(true);
							post({ action: "oauth-logout" }).then(function () { return post({ action: "oauth-start" }); }).then(function (res) {
								setOauthBusy(false);
							...
							}).catch(function () { setOauthBusy(false); if (win) win.close(); setErr("发起登录失败（网络）"); reload(); });
						} }, oauthBusy ? "发起中…" : "重新登录") : null,
						createElement(ConfirmButton, { variant: "outline", confirmLabel: "确认退出", confirmKey: "cb-logout", onConfirm: function () { post({ action: "oauth-logout" }).then(reload); } }, "退出登录")));
				...
				var rows = sortedKeys.map(function (k) {
					...
					return createElement("div", { key: k.name, className: "cbc-listrow" },
						...
						createElement(ConfirmButton, { variant: "ghost", confirmLabel: "确认删除", confirmKey: k.name, onConfirm: function () { post({ patch: { apiKeysRemove: k.name } }).then(reload); } }, "删除"));
				});
				body = rows.concat([
					createElement("div", { key: "add", className: "cbc-addrow" },
						...
						createElement(CbcInput, { type: "password", autoComplete: "off", widthClass: "cbc-w260", placeholder: "ck_…", value: addKey, onInput: function (e) { setAddKey(e.target.value); } }),
			...
			return createElement("div", { className: "cbc-section" },
				...
				providers.map(function (p) {
					return createElement("div", { key: p.id, className: "cbc-listrow" },
						...
						createElement(ConfirmButton, { variant: "ghost", confirmLabel: "确认删除", confirmKey: p.id, disabled: busy, onConfirm: function () { removeUpstream(p.id); } }, "删除"));
				}),
				createElement("div", { className: "cbc-row" },
					...
					createElement("div", { className: "cbc-row-control" },
						...
						createElement(CbcInput, { type: "password", autoComplete: "off", widthClass: "cbc-w200", placeholder: "API Key", value: addKey, onInput: function (e) { setAddKey(e.target.value); } }),
		...
		function TraeSection(props) {
			...
			return createElement("div", { className: "cbc-section" },
				...
				createElement("div", { className: "cbc-row" },
					...
					createElement("div", { className: "cbc-row-control" },
						toauth.signedIn
							? createElement(ConfirmButton, { variant: "outline", confirmLabel: "确认退出", confirmKey: "trae-logout", onConfirm: function () { post({ action: "trae-oauth-logout" }).then(reload); } }, "退出登录")
				...
				createElement("div", { className: "cbc-row" },
					...
					createElement("div", { className: "cbc-row-control" },
						...
						createElement("span", { className: "cbc-muted" }, "出站护栏（Trae inline 与 CodeBuddy 流式桥共用）：响应头超时即快速失败（SSE 长流不受限）"))),
				createElement("div", { className: "cbc-row" },
